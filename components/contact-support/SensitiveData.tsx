export default function SensitiveData() {
  return (
    <section className="w-full bg-[#091127]">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col items-start px-5 py-14 sm:px-8 sm:py-16 md:px-10 md:py-20 lg:px-14 xl:px-24">
        <div className="mx-auto flex w-full max-w-[1240px] flex-col items-center gap-11 px-7">
          
          {/* SECTION INTRO */}
          <div className="flex w-full max-w-[1000px] flex-col items-center gap-3 pt-2 text-center">
            
            {/* EYEBROW */}
            <div className="flex h-4 items-center justify-center gap-3">
              <span className="h-px w-4 shrink-0 bg-white opacity-40" />

              <span className="text-xs font-bold uppercase leading-4 tracking-widest text-white/55">
                Sensitive data &amp; secret handling
              </span>

              <span className="h-px w-4 shrink-0 bg-white opacity-40" />
            </div>

            {/* HEADING */}
            <h2
              className="
                !m-0
                w-full
                text-center
                !text-[30px]
                !font-extrabold
                !leading-[1.2]
                !tracking-[-0.035em]
                !text-white
                sm:!text-[34px]
                md:!text-[36px]
                lg:!text-[40px]
              "
            >
              What a general support form must
              <br className="hidden sm:block" />
              never receive.
            </h2>

            {/* DESCRIPTION */}
            <p
              className="
                !m-0
                w-full
                max-w-[605px]
                pt-[3px]
                text-center
                text-[15px]
                font-normal
                leading-7
                text-white/72
                sm:text-base
              "
            >
              Support intake is where customers paste whatever they think might
              help. The defence is refusing it, detecting it, and never storing
              it.
            </p>
          </div>

          {/* IMAGE */}
          <div className="w-full overflow-hidden rounded-2xl border border-[#dfe5ee] shadow-[0_8px_24px_rgba(15,23,42,0.05),0_1px_2px_rgba(15,23,42,0.04)]">
            <img
              src="/images/contact-support/sensitive-data.png"
              alt="Sensitive data and secret handling"
              className="block h-auto w-full"
            />
          </div>
        </div>
      </div>
    </section>
  );
}